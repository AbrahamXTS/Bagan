...
export default function Existencias({existencias}) {
    ...
    const filterResults = (busqueda, existencias) => {
        if (busqueda == "") {
            setExistenciasFiltradas(existencias);
        } else {
            setExistenciasFiltradas(existencias.filter((existencia) => existencia.nombre_articulo.toLowerCase().includes(busqueda)));
        }
    }
    
    useEffect(() => {
        filterResults(busqueda, existencias);
    }, [busqueda]);
    ...
    return (
        <>
            ...
            <h1 className="text-center uppercase text-[27px] font-bold py-8">Tabla de existencias</h1>
	
            <Buscador setBusqueda={setBusqueda} />

            {existenciasFiltradas.map((producto, index) => {

                const {promedio, clave_articulo, nombre_articulo, tipo_unidad, cantidad, partidas} = producto;
...
export async function getServerSideProps() {
    let res = await fetch("https://cecatirestapi-production.up.railway.app/existencias");
