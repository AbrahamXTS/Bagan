...
export default function salida({areas, productosBD}) {
  ...
  const handleProductsSubmit = ({productos}) => {
    ...
    if (error) {
      ...
      const options = { method: "POST", body: JSON.stringify({fecha, solicitante, productos, totalSalida}), headers: {'Content-Type':'application/json'}}
	...
	return (
		<>
      <Head>
				<title>Salida de articulos</title>
